...
const phishingWorkflowSchema = z.object({
  ...
  isQuishing: z
    ...
    .optional()
    .describe(
      'Whether this is a quishing (QR code phishing) simulation. Set to true if user explicitly requests quishing/QR code phishing.'
    ),
  targetProfile: z
    .object({
  ...
  additionalContext: z
    ...
    .optional()
    .describe(
      'Strategic context from Agent reasoning (e.g. "Use Authority trigger", "Focus on Fear", "Simulate CEO"). Also used for vulnerability analysis details.'
    ),
    ...
    try {
      ...
      if (result.status === 'success' && result.result) {
        ...
        const toolResult = {
          ...
          status: 'success',
          message:
            '[SUCCESS] Phishing simulation generated successfully. Phishing ID: ' +
            output.phishingId +
            '. **STOP - Do NOT call this tool again. The simulation is complete.**',
