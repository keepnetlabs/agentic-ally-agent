    ...
    describe('parseAndValidateEmailResponse', () => {
        ...
        it('should parse img src entries in template for logging path', () => {
            const responseWithImages = JSON.stringify({
                ...
                template: "<html><body><img src='https://a.example/x.png' /><img src=\"https://b.example/y.png\" /></body></html>",
        ...
        it('uses preserveLandingFormControlStyles in translate mode', async () => {
            const landingResult = {
                ...
                value: {
                    text: JSON.stringify({
                        type: 'login',
                        template: '<html><body><form><input name=\"u\" /><button type=\"submit\">Continue Login Securely</button></form></body></html>',
                        edited: true,
        ...
        it('uses preserveMissingLandingFormControlStyles when edit instruction targets form styles', async () => {
            const landingResult = {
                ...
                value: {
                    text: JSON.stringify({
                        type: 'success',
                        template: '<html><body><button class=\"cta\">Proceed to Secure Dashboard Confirmation</button></body></html>',
                        edited: true,
                        summary: 'changed'
                    })
                }
        ...
        it('falls back to repaired html when original template is missing', async () => {
            const landingResult = {
                ...
                value: {
                    text: JSON.stringify({
                        type: 'info',
                        template: '<html><body><p>This is a fresh informational landing page template with enough content.</p></body></html>',
                        edited: true,
                        summary: 'fresh'
                    })
                }
