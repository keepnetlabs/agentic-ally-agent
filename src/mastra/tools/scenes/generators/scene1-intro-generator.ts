    ...
    "highlights": [
      {
        ...
        "text": "KNOW statement (≤8 words): Start with 'Know that'. Topic and Department-aware impact. Pattern: [topic fact] [impacts department outcome]. Department focus: Finance→fraud/money | Operations/IT→system/reliability | HR/General→team/safety. CRITICAL: Write naturally in ${analysis.language} as a native speaker would. For English, use 'Know that...'. For other languages, use natural expressions (NOT literal translations). Examples: Phishing+Finance→'Know that phishing targets payment requests' | Ransomware+Operations→'Know that ransomware stops systems' | Password+IT→'Know that weak passwords invite attacks'. Turkish→'Oltalama saldırıları BT sistemlerini hedef alır' (NOT 'Bilin ki...'). Generate contextually for ${analysis.topic} + ${analysis.department} in ${analysis.language}."
      },
      {
        "iconName": "users",
        "text": "REMEMBER statement (≤8 words): Start with 'Remember that'. Topic and Department-aware empowering action. Pattern: [action/strategy] [prevents/protects/stops] [outcome]. Department focus: Finance→fraud/money/verification | Operations/IT→system/speed/reliability | HR/General→team/safety. CRITICAL: Write naturally in ${analysis.language} as a native speaker would. For English, use 'Remember that...'. For other languages, use natural expressions (NOT literal translations). Examples: Phishing+Finance→'Remember that verification prevents fraud' | Phishing+IT→'Remember that quick action stops spread' | Phishing+General→'Remember that anyone can be targeted' | Password+IT→'Remember that strong passwords stop breaches'. Turkish→'Doğrulama dolandırıcılığı önler' (NOT 'Unutmayın ki...'). Use conversational language. Generate contextually for ${analysis.topic} + ${analysis.department} in ${analysis.language}."
      },
      {
        "iconName": "shield-check",
        "text": "SEE statement (≤8 words): Start with 'See how'. Topic and Department-aware positive outcome. Pattern: [learner action] [leads to/enables/protects] [positive outcome]. Department focus: Finance→transaction/asset protection | Operations/IT→system/reliability | HR/General→team/colleague safety. CRITICAL: Write naturally in ${analysis.language} as a native speaker would. For English, use 'See how...'. For other languages, use natural expressions (NOT literal translations). Examples: Phishing+Finance→'See how verification protects transactions' | Ransomware+Operations→'See how backups save systems' | Password+IT→'See how strong passwords enable security' | MFA+General→'See how MFA protects access'. Turkish→'Doğrulama işlemleri nasıl korur' (NOT 'Görün ki...'). Generate contextually for ${analysis.topic} + ${analysis.department} in ${analysis.language}."
      }
    ],
    "key_message": [
