...
describe('Scene 4 - Actionable Rewriter', () => {
  ...
  describe('Function Call', () => {
    it('should accept valid scene and context', async () => {
      const result = await rewriteScene4Actionable(baseScene, baseContext);
    ...
    it('should require context parameter', async () => {
      await expect(
        rewriteScene4Actionable(baseScene, undefined as any)
      ).rejects.toThrow();
    });
  });
  ...
  describe('Scene Structure Preservation', () => {
    ...
    it('should preserve actionItems array', async () => {
      ...
      expect(result.actionItems.length).toBe(baseScene.actionItems.length);
    });
  ...
  describe('Language Support', () => {
    it('should support Turkish rewriting', async () => {
      ...
      const result = await rewriteScene4Actionable(baseScene, context);
      expect(result).toBeDefined();
    });
  ...
  describe('Edge Cases', () => {
    it('should handle single action item', async () => {
      ...
      const result = await rewriteScene4Actionable(singleAction, baseContext);
      expect(result).toBeDefined();
    });
    ...
    it('should handle many action items', async () => {
      ...
      const result = await rewriteScene4Actionable(manyActions, baseContext);
      expect(result).toBeDefined();
    });
    ...
    it('should handle long guidance text', async () => {
      ...
      const result = await rewriteScene4Actionable(longScene, baseContext);
      expect(result).toBeDefined();
    });
  });
  ...
  describe('Async Behavior', () => {
    it('should return a promise', () => {
      const result = rewriteScene4Actionable(baseScene, baseContext);
      expect(result).toBeInstanceOf(Promise);
    });

    it('should handle concurrent calls', async () => {
      const results = await Promise.all([
        rewriteScene4Actionable(baseScene, { ...baseContext, targetLanguage: 'tr' }),
        rewriteScene4Actionable(baseScene, { ...baseContext, targetLanguage: 'de' }),
      ]);
