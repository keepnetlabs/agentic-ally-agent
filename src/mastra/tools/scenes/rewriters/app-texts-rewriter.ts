...
- Preserve JSON structure, IDs, URLs, booleans, numbers.
...
- Context: ${department || 'General'} employees.
- Content Type: Application UI Texts - Use common UI patterns and natural phrasing.
...
- Does every sentence sound like it was naturally written in ${targetLanguage}, not translated?`;
    ...
    const userPrompt = `=== RAW CONTENT CONCEPTS (Source) ===
...
Output (JSON only):`;
    ...
    try {
        ...
        const errorInfo = errorService.aiModel(`App Texts rewrite failed: ${err.message}`, { stack: err.stack });
        logErrorInfo(logger, 'error', 'App Texts rewrite failed', errorInfo);
        throw error;
    }
}
