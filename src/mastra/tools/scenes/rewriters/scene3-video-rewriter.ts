...
export async function rewriteScene3Video(scene: any, context: RewriteContext): Promise<any> {
    const logger = getLogger('RewriteScene3Video');
...
- Preserve timestamps in transcript (00:00:00, 00:00:04, etc.) unchanged.
- Preserve \\n line breaks in transcript unchanged.
...
- Scene Type: Video/Scenario - Keep transcript natural and engaging.
...
Output (JSON only):`;
    ...
    try {
        ...
        const errorInfo = errorService.aiModel(`Scene 3 (Video) rewrite failed: ${err.message}`, { scene: 3, stack: err.stack });
        logErrorInfo(logger, 'error', 'Scene 3 (Video) rewrite failed', errorInfo);
        throw error;
    }
}
