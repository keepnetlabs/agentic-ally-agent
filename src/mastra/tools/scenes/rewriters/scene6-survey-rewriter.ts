...
export async function rewriteScene6Survey(scene: any, context: RewriteContext): Promise<any> {
    const logger = getLogger('RewriteScene6Survey');
...
- Scene Type: Survey - Make questions neutral and clear.
...
Output (JSON only):`;
    ...
    try {
        ...
        const errorInfo = errorService.aiModel(`Scene 6 (Survey) rewrite failed: ${err.message}`, { scene: 6, stack: err.stack });
        logErrorInfo(logger, 'error', 'Scene 6 (Survey) rewrite failed', errorInfo);
        throw error;
    }
}
