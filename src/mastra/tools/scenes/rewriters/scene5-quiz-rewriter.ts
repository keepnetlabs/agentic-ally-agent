...
- Preserve boolean values like isCorrect (must stay exactly the same - CRITICAL).
...
- Scene Type: Quiz - Keep questions clear and natural.
...
- Did I preserve isCorrect booleans unchanged (CRITICAL)?
...
CRITICAL: Keep isCorrect booleans unchanged.
...
Output (JSON only):`;
    ...
    try {
        ...
        const errorInfo = errorService.aiModel(`Scene 5 (Quiz) rewrite failed: ${err.message}`, { scene: 5, stack: err.stack });
        logErrorInfo(logger, 'error', 'Scene 5 (Quiz) rewrite failed', errorInfo);
        throw error;
    }
}
