...
export async function rewriteScene7Nudge(scene: any, context: RewriteContext): Promise<any> {
    const logger = getLogger('RewriteScene7Nudge');
...
- Scene Type: Nudge - Make it motivational and engaging.
...
Output (JSON only):`;
    ...
    try {
        ...
        const errorInfo = errorService.aiModel(`Scene 7 (Nudge) rewrite failed: ${err.message}`, { scene: 7, stack: err.stack });
        logErrorInfo(logger, 'error', 'Scene 7 (Nudge) rewrite failed', errorInfo);
        throw error;
    }
}
