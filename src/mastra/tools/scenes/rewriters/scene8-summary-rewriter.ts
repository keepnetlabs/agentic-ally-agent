...
export async function rewriteScene8Summary(scene: any, context: RewriteContext): Promise<any> {
    const logger = getLogger('RewriteScene8Summary');
...
- Scene Type: Summary - Make key points memorable and clear.
...
- Rewrite resource "title" values in natural ${targetLanguage} (max 5 words, max 12 words for descriptions)
- NEVER change "type" (keep as "URL") or "url" values
- Titles should be topic-specific and concise in ${targetLanguage}
...
Output (JSON only):`;
    ...
    try {
        ...
        const errorInfo = errorService.aiModel(`Scene 8 (Summary) rewrite failed: ${err.message}`, { scene: 8, stack: err.stack });
        logErrorInfo(logger, 'error', 'Scene 8 (Summary) rewrite failed', errorInfo);
        throw error;
    }
}
