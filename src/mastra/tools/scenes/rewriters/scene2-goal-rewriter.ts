...
export async function rewriteScene2Goal(scene: any, context: RewriteContext): Promise<any> {
    const logger = getLogger('RewriteScene2Goal');
...
- Scene Type: Goals - Make goals clear and measurable.
...
Output (JSON only):`;
    ...
    try {
        ...
        const errorInfo = errorService.aiModel(`Scene 2 (Goals) rewrite failed: ${err.message}`, { scene: 2, stack: err.stack });
        logErrorInfo(logger, 'error', 'Scene 2 (Goals) rewrite failed', errorInfo);
        throw error;
    }
}
