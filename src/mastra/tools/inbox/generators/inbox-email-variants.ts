...
export const variantDeltaBuilder: Record<EmailVariant, (d: DiversityHints) => string> = {
  [EmailVariant.ObviousPhishing]: d => {
    const departmentContext = d.departmentHint
      ? `Department scenario: Someone from ${d.departmentHint} who would realistically contact recipient.`
      : '';
    ...
    const impersonationHint =
      d.topicHint?.includes('executive') || d.topicHint?.includes('CEO') || d.topicHint?.includes('authority')
  ...
  [EmailVariant.SophisticatedPhishing]: d => {
    const departmentContext = d.departmentHint
      ? `Department context: ${d.departmentHint}. Impersonate colleague/authority from this department who would realistically interact with recipient.`
      : '';
    ...
    const impersonationHint =
      d.topicHint?.includes('executive') ||
      d.topicHint?.includes('CEO') ||
      d.topicHint?.includes('colleague') ||
      d.topicHint?.includes('authority')
  ...
  [EmailVariant.CasualLegit]: d => {
    const departmentContext = d.departmentHint
      ? `From ${d.departmentHint} department. Use friendly, informal tone typical of this department's internal communications.`
      : '';
    ...
    const contextHint =
      d.topicHint?.includes('account verification') || d.topicHint?.includes('login activity')
  ...
  [EmailVariant.FormalLegit]: d => {
    const departmentContext = d.departmentHint
      ? `From ${d.departmentHint} department leadership/authority. Use formal, professional, policy-focused tone typical of this department's official announcements.`
      : '';
    ...
    const contextHint =
      d.topicHint?.includes('executive') || d.topicHint?.includes('CEO')
  ...
  const plans = [
    {
      ...
      domainHint:
        'billing@invoice-systems.net, accounting@payments-hub.org, support@verify-account.net, admin@confirm-identity.net, security@process-payment.io',
    ...
    },
    {
      ...
      domainHint:
        'finance@acme-solutions.net, support@global-services.com, hr@corporate-group.io, operations@business-portal.net, compliance@allied-services.org',
    ...
    },
    {
      ...
      domainHint:
        'Pick ONE: it@company.com, communications@company.com, operations@company.com, facilities@company.com, marketing@company.com',
    ...
    },
    {
      ...
      domainHint:
        'Pick ONE: compliance@company.com, security@company.com, legal@company.com, finance@company.com, audit@company.com',
...
function getTopicHint(topic: string, index: number): string {
  ...
  if (
    t.includes('vishing') ||
    t.includes('voice phishing') ||
    (t.includes('phone') && (t.includes('scam') || t.includes('fraud')))
  ) {
    const scenarios = [
  ...
  if (
    t.includes('smishing') ||
    t.includes('sms phishing') ||
    (t.includes('text') && (t.includes('scam') || t.includes('fraud')))
  ) {
    const scenarios = [
  ...
  if (
    (t.includes('social') && t.includes('engineering')) ||
  ...
  if (
    t.includes('credential stuffing') ||
    t.includes('account takeover') ||
    (t.includes('password') && t.includes('reuse'))
  ) {
    const scenarios = [
  ...
  if (
    t.includes('shoulder surfing') ||
    t.includes('visual hacking') ||
    (t.includes('physical') && t.includes('privacy'))
  ) {
    const scenarios = [
  ...
  if (
    t.includes('public wifi') ||
    t.includes('wi-fi security') ||
    t.includes('public wi-fi') ||
    (t.includes('network') && t.includes('public'))
  ) {
    const scenarios = [
  ...
  if (
    t.includes('iot') ||
    t.includes('smart device') ||
    t.includes('connected device') ||
...
export function buildHintsFromInsights(
  topic: string,
  index: number,
  department?: string,
