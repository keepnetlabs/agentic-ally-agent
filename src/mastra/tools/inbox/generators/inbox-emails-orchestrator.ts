...
async function generateOneEmail(
  ...
  const delta =
    variantDeltaBuilder[variant](buildHintsFromInsights(topic, index, department, additionalContext)) +
    ` ${timestampInstructionText}`;
  ...
  const isPhishingVariant = variant === EmailVariant.SophisticatedPhishing || variant === EmailVariant.ObviousPhishing;
...
export async function generateInboxEmailsParallel(args: OrchestratorArgs): Promise<SimulatedEmail[]> {
  ...
  const system = buildInboxEmailBaseSystem(args.topic, args.languageCode, args.category, args.riskArea, args.level);
  ...
  const tasks = variantPlan.map((variant, i) => {
    ...
    const useTargetedPrompt =
      variant === EmailVariant.SophisticatedPhishing || variant === EmailVariant.ObviousPhishing;
    const contextForVariant = useTargetedPrompt ? args.additionalContext : undefined;

    return generateOneEmail(
      i,
      system,
      args.model,
      args.topic,
