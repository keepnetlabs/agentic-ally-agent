...
function bindTranslatedStrings(original: unknown, extracted: ExtractedString[], translated: string[]): unknown {
  if (extracted.length !== translated.length) {
    ...
    const errorInfo = errorService.validation(
      `Mismatch: extracted ${extracted.length} strings but got ${translated.length} translations`,
      { extracted: extracted.length, translated: translated.length }
    );
...
function collectPlaceholders(s: string): string[] {
  return [...(s.match(ICU_TOKEN_RE) || []), ...(s.match(SIMPLE_PLACEHOLDER_RE) || [])].sort();
}
...
export const inboxTranslateJsonTool = createTool({
  ...
  description:
    'Translate inbox content (emails, texts) using simple chunk-based translation. Cost-effective approach for non-critical content.',
  ...
  execute: async (inputData: InboxTranslateInput, _ctx?: ToolExecutionContext) => {
    try {
      ...
      toolLogger.debug('Translation configuration', {
        protectedKeys,
        sourceLanguage,
        topic: topic || 'General',
        targetLanguage,
      ...
      async function translateChunk(chunk: ExtractedString[], chunkIndex: number): Promise<string[]> {
        ...
        try {
          ...
          for (let i = 0; i < chunk.length; i++) {
            ...
            if (tgt === undefined) {
              const errorInfo = errorService.aiModel(`Missing key "${i}" in translation output`, {
                chunkNumber,
                index: i,
              });
              throw new Error(errorInfo.message);
          ...
          const errorInfo = errorService.aiModel(err.message, {
            step: 'inbox-chunk-translation',
            chunkNumber,
            stack: err.stack,
          });
          logErrorInfo(toolLogger, 'warn', 'Chunk translation failed, using originals', errorInfo);
      ...
      for (let start = 0; start < chunks.length; start += BATCH_SIZE) {
        ...
        results.forEach((result, idx) => {
          if (result.status === 'fulfilled') {
            ...
            const errorInfo = errorService.aiModel(err.message, {
              step: 'inbox-chunk-translation-settled',
              batchStart: start,
              chunkIndex: idx,
            });
            logErrorInfo(toolLogger, 'warn', 'Chunk translation promise rejected', errorInfo);
      ...
      if (allTranslated.length !== extracted.length) {
        const errorInfo = errorService.validation(
          `Total mismatch: expected ${extracted.length}, got ${allTranslated.length}`,
          { expected: extracted.length, got: allTranslated.length }
        );
      ...
      return {
        success: true,
        data: result,
        error: issues.length ? `Completed with ${issues.length} soft issues` : undefined,
      };
    } catch (error) {
