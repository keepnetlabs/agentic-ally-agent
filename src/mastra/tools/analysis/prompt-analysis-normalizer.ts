...
function normalizeLevel(input: unknown): (typeof TRAINING_LEVELS)[number] {
  ...
  const mapped = lower.startsWith('beg')
    ? 'Beginner'
...
function normalizeSingleRole(input: unknown): (typeof ROLES)['VALUES'][number] {
  ...
  return candidate && values.includes(candidate) ? (candidate as (typeof ROLES)['VALUES'][number]) : ROLES.DEFAULT;
}
...
export function autoRepairPromptAnalysis(
  ...
  const duration =
    typeof analysis.duration === 'number' && Number.isFinite(analysis.duration)
      ? analysis.duration
  ...
  const learningObjectives = normalizeStringArray(analysis.learningObjectives, [
  ...
  const deliveryChannel = isSmishing ? (normalizeSmishingChannel(analysis.deliveryChannel) ?? 'sms') : undefined;
