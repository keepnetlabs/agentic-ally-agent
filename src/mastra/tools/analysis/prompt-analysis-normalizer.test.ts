  ...
  describe('autoRepairPromptAnalysis', () => {
    ...
    it('should derive keyTopics and practicalApplications from additionalContext when missing', () => {
      const analysis = {
        ...
        additionalContext:
          'Focus on CEO spoofing emails. Include invoice fraud examples. Cover urgent payment language.',
      } as any;
    ...
    it('should derive mustKeepDetails from context when missing', () => {
      const analysis = {
        topic: 'Phishing Awareness',
        additionalContext:
          'Must include invoice fraud. Must include spoofed CEO identity. Prioritize urgent payment pressure examples.',
      } as any;
    ...
    it('should preserve explicit mustKeepDetails over context-derived values', () => {
      ...
      expect(result.mustKeepDetails).toEqual(['Keep CFO invoice fraud scenario', 'Keep escalation path mention']);
    });
