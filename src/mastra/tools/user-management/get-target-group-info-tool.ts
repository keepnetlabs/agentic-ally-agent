...
export const getTargetGroupInfoTool = createTool({
    ...
    description: 'Resolves a target group resource ID (and department) by searching the platform when only a group name is known.',
    inputSchema: z
        .object({
            ...
            groupName: z.string().optional().describe('Group display name (e.g., "IT Department") used to search for the ID.'),
            departmentName: z.string().optional().describe('Optional department hint to attach to the resolved group.'),
        })
        .refine(
            (data) => Boolean(data.targetGroupResourceId) || Boolean(data.groupName?.trim()),
    ...
    execute: async ({ context, writer }) => {
        ...
        if (providedId) {
            ...
            if (!isSafeId(normalizedId)) {
                const errorInfo = errorService.validation('Invalid targetGroupResourceId format.', { field: 'targetGroupResourceId' });
        ...
        if (normalizedGroupName.length < MIN_GROUP_NAME_LENGTH) {
            const errorInfo = errorService.validation('Group name must be at least 3 characters long.', { field: 'groupName' });
