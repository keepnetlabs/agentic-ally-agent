...
export async function fetchUsersWithFilters(
  ...
  if (!filterGroup?.FilterItems) {
    const errorInfo = errorService.validation('Invalid GET_ALL payload template (missing filter group)', {
      step: 'fetchUsersWithFilters',
    });
...
async function tryEnrichWithPhone(
  ...
  try {
    const enriched = await withRetry(() => fetchUserByIdDirect(deps, resourceId), 'phone_enrichment_direct_lookup', {
      maxAttempts: 2,
    });
...
async function fetchUserByIdDirect(deps: UserSearchDeps, targetUserResourceId: string): Promise<PlatformUser | null> {
  const { token, companyId, baseApiUrl, logger } = deps;
  const headers: Record<string, string> = {
    'Content-Type': 'application/json',
    Authorization: `Bearer ${token}`,
...
export async function findUserById(
  ...
  try {
    ...
    if (!fallbackExact.phoneNumber) {
      ...
      const merged = await tryEnrichWithPhone(fallbackExact, deps, normalizedId, {
        targetUserResourceId: normalizedId,
      });
      if (merged) return merged;
    }
