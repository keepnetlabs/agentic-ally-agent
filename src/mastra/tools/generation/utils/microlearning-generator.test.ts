...
import {
  ...
  enhanceMicrolearningContent,
  generateMicrolearningJsonWithAI,
...
describe('microlearning-generator', () => {
  ...
  describe('generateMicrolearningJsonWithAI', () => {
    ...
    it('should handle theme color correctly', async () => {
      (ai.generateText as any).mockResolvedValue({
        text: JSON.stringify({ theme: { colors: { background: 'red' } } }),
      });

      await generateMicrolearningJsonWithAI(
        { ...baseAnalysis, themeColor: 'custom-bg' } as any,
        'id',
        mockModel as any
      );
