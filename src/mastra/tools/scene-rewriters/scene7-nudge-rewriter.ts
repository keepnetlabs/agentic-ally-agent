...
export async function rewriteScene7Nudge(scene: any, context: RewriteContext): Promise<any> {
    ...
    const systemPrompt = `You are a ${targetLanguage} cybersecurity trainer creating motivational nudge FROM SCRATCH.
...
3. Think like a native ${targetLanguage} motivational coach
...
- Rewrite all text naturally - motivating and encouraging
...
Rewrite ALL text values in native ${targetLanguage}. Make it motivational.
...
Output (JSON only):`;
    ...
    try {
        ...
        const cleaned = cleanResponse(response.text, 'scene7-nudge');
        const rewritten = JSON.parse(cleaned);

        return rewritten;
    } catch (error) {
        console.error('❌ Scene 7 (Nudge) rewrite failed:', error);
        throw error;
    }
}
