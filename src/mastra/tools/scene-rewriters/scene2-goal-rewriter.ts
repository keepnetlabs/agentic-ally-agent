...
export async function rewriteScene2Goal(scene: any, context: RewriteContext): Promise<any> {
    ...
    const systemPrompt = `You are a ${targetLanguage} cybersecurity trainer creating learning goals FROM SCRATCH.
...
- Rewrite all text naturally - clear and professional
...
Rewrite ALL text values in native ${targetLanguage}. Make goals clear and measurable.
...
Output (JSON only):`;
    ...
    try {
        ...
        const cleaned = cleanResponse(response.text, 'scene2-goal');
