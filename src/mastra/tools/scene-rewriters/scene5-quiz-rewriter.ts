...
export async function rewriteScene5Quiz(scene: any, context: RewriteContext): Promise<any> {
    ...
    const systemPrompt = `You are a ${targetLanguage} cybersecurity trainer creating quiz FROM SCRATCH.
...
3. Think like a native ${targetLanguage} educator

What to rewrite:
- ALL string values in the JSON (questions, answers, explanations)

What NOT to change:
- JSON keys
- URLs, IDs, scene_type
- CRITICAL: Boolean values like isCorrect (true/false must stay exactly the same)

Rules:
- Keep JSON structure identical
- Rewrite all text naturally - clear and unambiguous
- NEVER change isCorrect boolean values
...
Rewrite ALL text values in native ${targetLanguage}. CRITICAL: Keep isCorrect booleans unchanged.
