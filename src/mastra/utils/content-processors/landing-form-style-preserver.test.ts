...
} from './landing-form-style-preserver';
...
describe('preserveMissingLandingFormControlStyles', () => {
  it('restores original style only when edited style is missing/broken', () => {
    const original = `<input id='email' type='email' style='padding: 12px; border: 1px solid #ddd;' />`;
    const editedBroken = `<input id="email" type="email" style=" 1px=" "="" solid="" 11px="" #f9fafb'="">`;
    const out = preserveMissingLandingFormControlStyles(original, editedBroken);
    expect(out).toContain("style='padding: 12px; border: 1px solid #ddd;'");
  });

  it('does not override a valid edited style', () => {
    const original = `<input id='email' type='email' style='padding: 12px; border: 1px solid #ddd;' />`;
    const editedValid = `<input id="email" type="email" style="padding: 8px; border: 2px solid red;" />`;
    const out = preserveMissingLandingFormControlStyles(original, editedValid);
    expect(out).toContain("style='padding: 8px; border: 2px solid red;'");
    expect(out).not.toContain("style='padding: 12px; border: 1px solid #ddd;'");
  });
});

describe('repairBrokenLandingFormControlAttrs', () => {
  it('repairs broken boolean attrs like required=\" style=\"...\"', () => {
    const broken = `<input id="email" type="email" name="email" placeholder="you@example.com" autocomplete="email" required=" style="width:100%;border-radius:10px;" />`;

    const out = repairBrokenLandingFormControlAttrs(broken);
    // required should become boolean, and style attribute should remain
    expect(out).toMatch(/\brequired\b/i);
    expect(out).toMatch(/\bstyle\s*=\s*(['"])/i);
    expect(out).not.toMatch(/\brequired\s*=\s*(['"])\s*(?=\bstyle\b\s*=)/i);
  });

  it('strips garbage attrs produced from broken inline styles', () => {
    const broken = `<input data-gjs-highlightable="true" id="email" type="email" name="email" placeholder="you@example.com" autocomplete="email" required="true" "="" border-radius:6px;="" border:2px="" solid="" #e5e7eb;="" padding:12px="" 14px;="" font-size:14px;="" background-color:#f9fafb;="" margin-bottom:16px;'="" class="">`;
    const out = repairBrokenLandingFormControlAttrs(broken);

    // should keep the tag and core attrs
    expect(out).toContain('<input');
    expect(out).toMatch(/\bid=["']email["']/);
    // required should be normalized to boolean
    expect(out).toMatch(/\brequired\b/i);

    // should NOT keep CSS-fragment attributes like border-radius:...=""
    expect(out).not.toMatch(/border-radius\s*:/i);
    expect(out).not.toMatch(/background-color\s*:/i);
    expect(out).not.toMatch(/\s"="\s*/);
  });

  it('does not emit broken style attributes (regression for style=\" 1px=\" ...)', () => {
    const broken = `<input id="email" type="email" name="email" placeholder="you@example.com" autocomplete="email" required="" style=" 1px=" "="" solid="" 11px="" #f9fafb'="">`;
    const out = repairBrokenLandingFormControlAttrs(broken);
    expect(out).toContain('<input');
    // should be valid-ish: no attribute fragments like 1px="" or solid=""
    expect(out).not.toMatch(/\b1px\b\s*=/i);
    expect(out).not.toMatch(/\bsolid\b\s*=/i);
  });
});
