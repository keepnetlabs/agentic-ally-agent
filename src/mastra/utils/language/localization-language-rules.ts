...
🚫 ACRONYM & TERM LOCALIZATION (Polish-specific):
  - "MFA" → "uwierzytelnianie wieloskładnikowe (MFA)" on first use, then "uwierzytelnianie wieloskładnikowe"
    ❌ "Weryfikacja MFA" → ✅ "Weryfikacja wieloskładnikowa"
    ❌ "Użyj przycisku MFA" → ✅ "Użyj uwierzytelniania wieloskładnikowego"
  - "2FA" → "uwierzytelnianie dwuskładnikowe"
  - "URL" → "link" or "adres URL" in user-facing text (NEVER bare "URL" as a label)
    ❌ "Sprawdź URL" → ✅ "Sprawdź link"
  - "SSO" → "jednokrotne logowanie (SSO)"
  - "VPN" → "VPN" (accepted loanword)
...
🚫 ANTI-TRANSLATIONESE (Persian-specific calque patterns to AVOID):
  These English sentence openers must NEVER be literally translated:
  - ❌ "Know that X" → "بدانید که X" — WRONG. State X directly: "احراز هویت ضعیف خطر نفوذ را افزایش می‌دهد"
  - ❌ "Remember that X" → "به‌خاطر بسپارید که X" — WRONG. State X directly: "تأیید درخواست‌های ناشناخته خطر از دست رفتن داده‌ها را دارد"
  - ❌ "See how X" → "مشاهده کنید که X" / "ببینید چگونه X" — WRONG. Express directly: "استفاده از برنامه احراز هویت حملات را متوقف می‌کند"
  RULE: In highlights, titles, and key messages — lead with the FACT or ACTION, not a meta-instruction to the reader.

🚫 ACRONYM & TERM LOCALIZATION (Persian-specific):
  - "MFA" → "احراز هویت چندمرحله‌ای (MFA)" on first use, then "احراز هویت چندمرحله‌ای"
    ❌ "تایید MFA" → ✅ "تایید احراز هویت چندمرحله‌ای"
    ❌ "استفاده از MFA" → ✅ "استفاده از احراز هویت چندمرحله‌ای"
  - "2FA" → "احراز هویت دومرحله‌ای"
  - "URL" → "لینک" in user-facing text (NEVER bare "URL")
    ❌ "بررسی URL" → ✅ "بررسی لینک"
  - "SSO" → "ورود یکپارچه"
  - "VPN" → "VPN" (accepted loanword)
...
📝 TERMINOLOGY & NATURALNESS (Chinese Simplified-specific):
  - "phishing" → "钓鱼攻击" or "网络钓鱼" (NEVER bare English "phishing" in Chinese text)
    ❌ "Phishing 报告者" → ✅ "钓鱼邮件举报"
    ❌ "启用 MFA 保护" → ✅ "启用多因素认证保护"
...
🚫 ACRONYM LOCALIZATION (Chinese-specific):
  - "MFA" → "多因素认证（MFA）" on first use, then "多因素认证" (NEVER bare "MFA" — non-IT users won't understand)
  - "2FA" → "双重验证"
  - "SSO" → "单点登录"
  - "VPN" → "VPN" (accepted loanword in Chinese IT context)
  - "USB" → "U盘" or "USB设备"
...
🛡️ ENGLISH LOANWORD RULES (Chinese-specific):
  Accepted English terms (keep as-is): VPN, Wi-Fi, CEO, IT, IP, HTTPS, DNS, API
  MUST localize (see ACRONYM LOCALIZATION above): MFA → 多因素认证, URL → 链接, SSO → 单点登录
...
🚫 ANTI-TRANSLATIONESE (Japanese-specific calque patterns to AVOID):
  These English sentence openers must NEVER be literally translated:
  - ❌ "Know that X" → "Xを認識してください" / "Xを知ってください" — WRONG. State X directly: "弱いMFAはアカウント侵害のリスクを高めます"
  - ❌ "Remember that X" → "Xを覚えておいてください" — WRONG. State X directly: "不明なプッシュ承認はデータ損失のリスクになります"
  - ❌ "See how X" → "Xをご確認ください" / "Xがどのように" — WRONG. Express cause-effect directly: "認証アプリの使用が攻撃を防止します"
  RULE: In highlights, titles, and key messages — lead with the FACT or ACTION, not a meta-instruction to the reader.
...
🚫 ACRONYM LOCALIZATION (Japanese-specific):
  - "MFA" → "多要素認証（MFA）" on first use, then "多要素認証" (NEVER bare "MFA" — general employees won't understand)
    ❌ "MFA の使用" → ✅ "多要素認証の使用"
    ❌ "MFA を有効化" → ✅ "多要素認証を有効化"
  - "2FA" → "二要素認証"
  - "SSO" → "シングルサインオン（SSO）" on first use, then "シングルサインオン"
  - "VPN" → "VPN" (accepted loanword in Japanese IT)
  - "USB" → "USBメモリ" or "USB機器"
...
🚫 ACRONYM & TERM LOCALIZATION (Korean-specific):
  - "URL" → "링크" in user-facing text (more accessible), "URL" acceptable only in technical/developer context
    ❌ "URL을 확인하세요" → ✅ "링크를 확인하세요"
  - "MFA" → "다중 인증（MFA）" on first use, then "다중 인증"
  - "2FA" → "이중 인증"
  - "SSO" → "통합 로그인"
  - "VPN" → "VPN" (accepted loanword)
  - "USB" → "USB" (accepted loanword)
...
🚫 ACRONYM & TERM LOCALIZATION (Thai-specific):
  - "MFA" → "การยืนยันตัวตนหลายปัจจัย (MFA)" on first use, then "การยืนยันตัวตนหลายปัจจัย"
    ❌ "ใช้ MFA" → ✅ "ใช้การยืนยันตัวตนหลายปัจจัย"
  - "2FA" → "การยืนยันตัวตนสองขั้นตอน"
  - "URL" → "ลิงก์" in user-facing text (NEVER bare "URL")
    ❌ "ตรวจสอบ URL" → ✅ "ตรวจสอบลิงก์"
  - "SSO" → "การเข้าสู่ระบบครั้งเดียว"
  - "VPN" → "VPN" (accepted loanword)
...
🚫 ACRONYM & TERM LOCALIZATION (Bengali-specific):
  - "MFA" → "মাল্টি-ফ্যাক্টর প্রমাণীকরণ (MFA)" on first use, then "মাল্টি-ফ্যাক্টর প্রমাণীকরণ"
    ❌ "MFA ব্যবহার" → ✅ "মাল্টি-ফ্যাক্টর প্রমাণীকরণ ব্যবহার"
  - "2FA" → "দুই-ধাপ প্রমাণীকরণ"
  - "URL" → "লিঙ্ক" in user-facing text
  - "SSO" → "একক সাইন-অন"
  - "VPN" → "VPN" (accepted loanword)
...
🚫 ANTI-TRANSLATIONESE (Tamil-specific calque patterns to AVOID):
  These English sentence openers must NEVER be literally translated into Tamil:
  - ❌ "Know that X" → "X என்பதை அறிந்து கொள்ளுங்கள்" — WRONG. State X directly: "பலவீனமான கடவுச்சொற்கள் கணக்கு ஊடுருவலுக்கு வழிவகுக்கும்"
  - ❌ "Remember that X" → "X என்பதை நினைவில் கொள்ளுங்கள்" — WRONG. State X directly: "அறியாத இணைப்புகளைக் கிளிக் செய்வது தரவு இழப்பை ஏற்படுத்தும்"
  - ❌ "See how X" → "X எப்படி என்று பாருங்கள்" — WRONG. Express directly: "பல காரணி அங்கீகாரம் தாக்குதல்களைத் தடுக்கிறது"
  - ❌ "Make sure to X" → "X செய்வதை உறுதிசெய்யுங்கள்" — Acceptable ONLY in procedural steps, not in highlights or titles.
  RULE: In highlights, titles, and key messages — lead with the FACT or ACTION, not a meta-instruction to the reader.

🚫 ACRONYM & TERM LOCALIZATION (Tamil-specific):
  - "MFA" → "பல காரணி அங்கீகாரம் (MFA)" on first use, then "பல காரணி அங்கீகாரம்"
  - "2FA" → "இரு காரணி அங்கீகாரம் (2FA)" on first use, then "இரு காரணி அங்கீகாரம்"
  - "SSO" → "ஒற்றை உள்நுழைவு (SSO)" on first use, then "ஒற்றை உள்நுழைவு"
  - "URL" → "இணைய முகவரி (URL)" on first use, then "இணைய முகவரி"
  - "VPN" → "மெய்நிகர் தனியார் வலையமைப்பு (VPN)" on first use, then VPN (accepted loanword)
  - RULE: On first occurrence, provide the Tamil term with the English abbreviation in parentheses. After that, use only the Tamil term.
...
🚫 ANTI-TRANSLATIONESE (Kazakh-specific calque patterns to AVOID):
  These English sentence openers must NEVER be literally translated into Kazakh:
  - ❌ "Know that X" → "X екенін біліңіз" — WRONG. State X directly: "Әлсіз құпия сөздер есептік жазбаны бұзуға әкеледі"
  - ❌ "Remember that X" → "X екенін есте сақтаңыз" — WRONG. State X directly: "Белгісіз сілтемелерді басу деректер жоғалуына әкеледі"
  - ❌ "See how X" → "X қалай екенін қараңыз" — WRONG. Express directly: "Көп факторлы аутентификация шабуылдарды тоқтатады"
  RULE: In highlights, titles, and key messages — lead with the FACT or ACTION, not a meta-instruction to the reader.

🚫 ACRONYM & TERM LOCALIZATION (Kazakh-specific):
  - "MFA" → "Көп факторлы аутентификация (MFA)" on first use, then "көп факторлы аутентификация"
  - "2FA" → "Екі факторлы аутентификация (2FA)" on first use, then "екі факторлы аутентификация"
  - "SSO" → "Бірыңғай кіру жүйесі (SSO)" on first use, then "бірыңғай кіру жүйесі"
  - "URL" → "интернет мекенжайы (URL)" on first use, then "интернет мекенжайы"
  - RULE: On first occurrence, provide the Kazakh term with the English abbreviation in parentheses. After that, use only the Kazakh term.
...
- Keep email protocol acronyms as-is: SPF, DMARC, DKIM, HTTPS, DNS.
- Keep accepted tech loanwords as-is: VPN, Wi-Fi, PDF, API, IT, IP.
- LOCALIZE user-facing security terms: MFA, 2FA, SSO, URL → use native equivalents per language-specific rules above.
- HIERARCHY: Language-specific rules (above) ALWAYS override this generic fallback.
