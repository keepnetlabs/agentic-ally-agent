...
describe('chat-request-helpers artifact id extraction', () => {
  ...
  describe('Basic Artifact Extraction', () => {
    it('extracts phishingId from phishing_email UI payload after routingContext cleaning', () => {
      const emailPayload = Buffer.from(JSON.stringify({ phishingId: 'ph-123', subject: 'x' }), 'utf-8').toString(
        'base64'
      );
    ...
    it('extracts microlearningId from training_meta UI payload after routingContext cleaning', () => {
      const trainingMeta = Buffer.from(
        JSON.stringify({ microlearningId: 'ml-999', trainingUrl: 'https://x/y' }),
        'utf-8'
      ).toString('base64');
  ...
  describe('Edge Cases', () => {
    ...
    it('extracts resourceId with minimum 3 characters', () => {
      const routingContext = buildRoutingContext([{ role: 'assistant', content: 'resourceId=abc' } as any]);
    ...
    it('ignores resourceId shorter than 3 characters', () => {
      const routingContext = buildRoutingContext([{ role: 'assistant', content: 'resourceId=ab' } as any]);
  ...
  describe('ID Format Variations', () => {
    it('extracts IDs with hyphens', () => {
      const routingContext = buildRoutingContext([{ role: 'assistant', content: 'resourceId=my-resource-123' } as any]);
    ...
    it('extracts IDs with underscores', () => {
      const routingContext = buildRoutingContext([{ role: 'assistant', content: 'resourceId=my_resource_456' } as any]);
    ...
    it('extracts IDs with mixed alphanumeric and symbols', () => {
      const routingContext = buildRoutingContext([{ role: 'assistant', content: 'resourceId=Res-123_ABC' } as any]);
    ...
    it('extracts numeric-only IDs', () => {
      const routingContext = buildRoutingContext([{ role: 'assistant', content: 'resourceId=123456' } as any]);
    ...
    it('extracts letter-only IDs', () => {
      const routingContext = buildRoutingContext([{ role: 'assistant', content: 'resourceId=abcdef' } as any]);
      ...
      const routingContext = buildRoutingContext([
        {
          role: 'assistant',
          content:
            'resourceId=res-123, scenarioResourceId=scenario-456, landingPageResourceId=landing-789, languageId=lang-abc, sendTrainingLanguageId=lang-def, targetUserResourceId=user-ghi, targetGroupResourceId=group-jkl, phishingId=ph-123456, microlearningId=ml-789012',
        } as any,
      ]);
  ...
  describe('Boundary Conditions', () => {
    it('handles very long IDs', () => {
      ...
      const routingContext = buildRoutingContext([{ role: 'assistant', content: `resourceId=${longId}` } as any]);
