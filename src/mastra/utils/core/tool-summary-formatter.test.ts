...
describe('tool-summary-formatter', () => {
  ...
  describe('formatToolSummary', () => {
    ...
    it('should handle missing optional parts', () => {
      ...
      // Prefix + KV
      expect(formatToolSummary({ prefix: 'Done', kv: [{ key: 'id', value: 1 }] })).toBe('Done. (id=1).');

      // Prefix + Title
      expect(formatToolSummary({ prefix: 'Done', title: 'File' })).toBe('Done: "File".');
    });

    it('should handle prefix + suffix without kv', () => {
      expect(formatToolSummary({ prefix: 'Uploaded', suffix: 'successfully' })).toBe('Uploaded. successfully.');
    });

    it('should handle prefix + title + suffix without kv', () => {
      expect(formatToolSummary({ prefix: 'Uploaded', title: 'image.png', suffix: 'successfully' })).toBe(
        'Uploaded: "image.png". successfully.'
      );
    });
    ...
    it('should trim whitespace from suffix', () => {
      expect(formatToolSummary({ prefix: 'Done', suffix: '  OK  ' })).toBe('Done. OK.');
    });
    ...
    it('should handle title with quotes', () => {
      expect(formatToolSummary({ prefix: 'Done', title: 'File "test"' })).toBe('Done: "File "test"".');
    });

    it('should handle title with special characters', () => {
      expect(formatToolSummary({ prefix: 'Done', title: 'file@2024.csv' })).toBe('Done: "file@2024.csv".');
    });
    ...
    it('should handle unicode in title', () => {
      expect(formatToolSummary({ prefix: 'Done', title: '文件.txt' })).toBe('Done: "文件.txt".');
    });
    ...
    it('should handle very long title', () => {
      ...
      expect(formatToolSummary({ prefix: 'Done', title: longTitle })).toBe(`Done: "${longTitle}".`);
    });

    it('should handle very long suffix', () => {
      const longSuffix = 'C'.repeat(1000);
      expect(formatToolSummary({ prefix: 'Done', suffix: longSuffix })).toBe(`Done. ${longSuffix}.`);
    });
    ...
    it('should handle all fields with complex data', () => {
      ...
      expect(result).toBe(
        '✅ Training uploaded: "Security Awareness 2024". Ready to assign (scenarioId=sc-123, duration=30, interactive=true).'
      );
    });
    ...
    it('should handle title with path separators', () => {
      expect(formatToolSummary({ prefix: 'Done', title: 'folder/subfolder/file.txt' })).toBe(
        'Done: "folder/subfolder/file.txt".'
      );
    });
    ...
    it('should handle title with backslashes', () => {
      expect(formatToolSummary({ prefix: 'Done', title: 'C:\\Users\\file.txt' })).toBe('Done: "C:\\Users\\file.txt".');
    });

    it('should handle prefix with multiple spaces', () => {
      expect(formatToolSummary({ prefix: 'Done    Task' })).toBe('Done    Task.');
    });
    ...
    it('should handle real-world phishing upload example', () => {
      ...
      expect(result).toBe(
        '✅ Phishing uploaded: "CEO Fraud Scenario". Ready to assign (scenarioName=CEO Fraud Scenario, resourceId=res-abc123, phishingId=ph-777777).'
      );
    });
    ...
    it('should handle undefined parameters gracefully', () => {
      ...
      expect(formatToolSummary({ prefix: 'Done', title: undefined, suffix: undefined, kv: undefined })).toBe('Done.');
    });

    it('should handle null title', () => {
      // @ts-ignore - Testing runtime behavior
      expect(formatToolSummary({ prefix: 'Done', title: null })).toBe('Done.');
    });

    it('should handle null suffix', () => {
      // @ts-ignore - Testing runtime behavior
      expect(formatToolSummary({ prefix: 'Done', suffix: null })).toBe('Done.');
    });
  });
});
