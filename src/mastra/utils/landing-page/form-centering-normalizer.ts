...
export function normalizeLandingFormCentering(html: string): string {
  ...
  try {
    ...
    try {
      ...
      logger.warn('⚠️ Fallback landing form centering normalization failed, returning original', {
        error: err2.message,
      });
      // Level 3: guaranteed
      return html;
...
export function normalizeLandingMaxWidthCentering(html: string): string {
  ...
  try {
    ...
    const err = normalizeError(error);
    logger.warn('⚠️ Primary landing max-width centering normalization failed, returning original', {
      error: err.message,
    });
    return html;
  }
...
export function enforceMinimalLayoutMaxWidth(html: string): string {
  ...
  try {
    const maxWidthPx = LANDING_PAGE.FORM_MAX_WIDTH_PX;
    let changed = false;
    const out = html.replace(/<form\b([^>]*)>/gi, (full, attrsRaw) => {
      const attrs = String(attrsRaw ?? '');
      const styleMatch = attrs.match(/\bstyle\s*=\s*(['"])(.*?)\1/i);
      ...
      if (!styleMatch) {
        const newStyle = `style='max-width: ${maxWidthPx}px; width: 100%; margin: 0 auto;'`;
        changed = true;
      ...
      const nextStyle =
        `max-width: ${maxWidthPx}px; width: 100%; margin: 0 auto; ${ensureTrailingSemicolon(style)}`.trim();
      const nextAttrs = attrs.replace(/\bstyle\s*=\s*(['"])(.*?)\1/i, `style='${nextStyle}'`);
      changed = true;
      return `<form${nextAttrs}>`;
    });
