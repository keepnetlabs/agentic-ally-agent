...
(Invalid JSON, skipped)`;
  ...
  private async loadFromD1Cache(): Promise<void> {
    ...
    try {
      ...
      const stmt = this.db.prepare('SELECT * FROM embedding_cache WHERE cache_version = ?').bind(this.cacheVersion);
      ...
      for (const doc of this.docs) {
        ...
        const cached = result.results.find((row: EmbeddingCacheDB) => row.path === doc.path);

        if (cached && cached.content_hash === contentHash) {
          // Parse cached data
          doc.embedding = JSON.parse(cached.embedding_json);
          doc.metadata = cached.metadata_json ? JSON.parse(cached.metadata_json) : this.extractMetadata(doc);
  ...
  private async performSemanticSearch(query: string, options: SemanticSearchOptions): Promise<SearchResult[]> {
    ...
    for (const doc of this.docs) {
      ...
      const tokenScore = options.useHybrid
        ? this.score(this.tokenize(query.toLowerCase()), (doc.path + ' ' + doc.content).toLowerCase())
        : 0;
  ...
  async getSmartSchemaHints(query?: string, maxFiles: number = 3): Promise<string> {
    ...
    const isComplexQuery =
      query && (query.length > 50 || /advanced|specific|detailed|custom/.test(query.toLowerCase()));
  ...
  async validateCache(): Promise<{
    ...
    try {
      for (const doc of this.docs) {
        ...
        const stmt = this.db.prepare('SELECT path, content_hash FROM embedding_cache WHERE path = ?');
  ...
  async optimizeCache(): Promise<{
    ...
    try {
      ...
      const allCached = await this.db.prepare('SELECT path FROM embedding_cache').all();
    ...
    try {
      ...
      if (topUsed.success && topUsed.results && topUsed.results.length > 0) {
        ...
        topUsed.results.forEach((entry: any, i: number) => {
          logger.info(
            `    ${i + 1}. ${entry.path} (used ${entry.usage_count} times, last: ${new Date(entry.last_used).toLocaleDateString()})`
          );
        });
      }
    } catch (error) {
