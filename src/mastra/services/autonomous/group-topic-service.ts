 ...
 */
export async function selectGroupTrainingTopic(_preferredLanguage: string | undefined): Promise<GroupTopicSelection> {
...
function getTopicMetadata(topic: string): { objectives: string[]; scenarios: string[] } {
  ...
  return (
    topicMetadata[topic] || {
      ...
      scenarios: ['Attack scenario for ' + topic, 'Defense awareness scenario', 'Best practice application scenario'],
    }
  );
}
