...
const IntroSceneSchema = BaseSceneContentSchema.extend({
  ...
  highlights: z
    ...
    .min(2)
    .max(5), // AI can decide 2-5 highlights based on content complexity
  duration: z.string(),
  level: z.string(),
});
...
const GoalSceneSchema = BaseSceneContentSchema.extend({
  ...
  goals: z
    ...
    .min(2)
    .max(4), // AI decides 2-4 goals based on training scope
});
...
const ActionableContentSceneSchema = BaseSceneContentSchema.extend({
  ...
  actions: z
    ...
    .min(2)
    .max(4), // AI decides 2-4 actions based on training complexity
  tipConfig: z.object({
    iconName: z.string(),
  }),
...
const VishingSimulationSceneSchema = BaseSceneContentSchema.extend({
  title: z.string(),
  subtitle: z.string(),
  callerName: z.string(),
...
const MultipleChoiceQuestionSchema = BaseQuizQuestionSchema.extend({
  ...
  options: z
    ...
    .min(2)
    .max(6), // AI can generate 2-6 options
});
...
const SummarySceneSchema = BaseSceneContentSchema.extend({
  immediateActions: z
    ...
    .min(1)
    .max(5), // AI can generate 1-5 immediate actions
  resources: z
    .array(
      z.object({
        title: z.string(),
    ...
    .min(0)
    .max(10), // AI can generate 0-10 additional resources
  texts: z
    .object({
