...
export const createSmishingInputSchema = z.object({
    ...
    additionalContext: z.string().optional().describe('User behavior context / vulnerability analysis for targeted smishing'),
...
export const createSmishingAnalysisSchema = z.object({
    ...
    description: z.string().max(PHISHING_EMAIL.MAX_DESCRIPTION_LENGTH).describe('Brief description of the smishing scenario'),
    category: z.string().describe('Smishing category: Credential Harvesting, Financial Fraud, etc.'),
    method: z.enum(SMISHING.ATTACK_METHODS).default(SMISHING.DEFAULT_ATTACK_METHOD).describe('The determined attack method for this scenario'),
    ...
    }).optional().describe('Authentic brand colors for recognized brands'),
